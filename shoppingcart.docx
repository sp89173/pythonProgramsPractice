class person:
      def __init__(self,name,age):
            self.Name = name
            self.Age = age

class student(person):
      def __init__(name,age = 706):
      self.roll_no = rollno

      def introduce(self):
            print(f"My name is {self.name},age{self.Age},roll number {self.roll_no}.")

            obj = student("raju",23)
            obj.introduce()


capstone project 2 – develop a python module to create a class representing a shopping cart. Include 
class ShoppingCart:
      def __init__(self):
            self.items = []
            
# add item
      def add_item(self,item_name,qty):
            item = (item_name,qty)
            self.items.append(item)
            
# remove item
      def remove_item(self,item_name):
            
            for item in self.items:
                  if item[0] == item_name:
                        self.items.remove(item)
                        break

                  #total
      def calculate_total(self):
            total = 0
            for item in self.items:
                  total+=item[1]
            return total
            
#driver code
cart = ShoppingCart()

# add item to the shoppingcart
cart.add_item("Papayya",100)
cart.add_item("Guava",200)
cart.add_item("orange",150)

print("current Items in Cart:")
for item in cart.items:
      print(item[0],"_",item[1])

total_qty = cart.calculate_total()
print("Total Quantity:",total_qty)

cart.remove_item("Papayya")
print(\nUpdated Items in Cart after removing Orange;")
for item in cart.items:
      print(item[0],"_",item[1])
total_qty = cart.calculate_total()
print("Total Quantity:",total_qty)

